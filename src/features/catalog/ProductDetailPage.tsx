...
import { Badge, Button, Card, CardBody, PageLoader, EmptyState } from "@/components/ui";
import { idr, num } from "@/lib/utils";
import { useAuthStore } from "@/stores/authStore";
import { useAddToCart } from "@/features/buyer/hooks";

export function ProductDetailPage({ basePath = "/products" }: { basePath?: string }) {
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-16">
        <EmptyState
          ...
          action={<Button onClick={() => navigate(basePath)}>Back to browse</Button>}
        />
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 lg:px-8">
      <nav className="mb-5 text-sm text-ink-400">
        <Link to={basePath} className="hover:text-brand-600">Browse</Link>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="relative aspect-square overflow-hidden rounded-2xl border border-ink-100 bg-gradient-to-br from-brand-100 to-brand-50">
            {product.imageUrl ? (
              ...
              <div className="grid h-full place-items-center text-brand-300">
                <svg viewBox="0 0 64 64" className="h-20 w-20" fill="currentColor" aria-hidden>
                  <path d="M8 38c8-12 20-17 32-17 9 0 16 5 16 5s-4 12-17 17c-12 5-24 1-31-5z" opacity="0.5" />
        ...
        {/* Details */}
        <div>
          <h1 className="font-display text-3xl font-bold text-ink-900">{product.name}</h1>
          {product.store && (
            <Link
          ...
          <p className="mt-5 font-display text-3xl font-bold text-ink-900">{idr(product.price)}</p>
          <div className="mt-2">
          ...
          {product.description && (
            <div className="mt-6">
              <h3 className="mb-1 text-sm font-semibold text-ink-700">Description</h3>
              ...
              {isBuyer ? (
                <>
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-ink-100 pt-4">
                    ...
                    <span className="font-display text-lg font-bold text-ink-900">{idr(lineTotal)}</span>
                  </div>
                  <Button
                    className="mt-4"
                    fullWidth
                ...
                <div className="text-center">
                  <p className="text-sm text-ink-500">Log in as a buyer to purchase this item.</p>
                  <Button className="mt-3" fullWidth onClick={() => navigate("/auth/login")}>
                    Log in to buy
                  </Button>
                </div>
              )}
