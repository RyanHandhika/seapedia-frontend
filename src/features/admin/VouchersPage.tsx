  ...
  const columns: Column<Voucher>[] = [
    {
      ...
      header: "Code",
      render: (v) => (
        <span className="font-mono font-medium text-ink-900">{v.code}</span>
      ),
    },
    {
      key: "type",
      header: "Type",
      render: (v) => <Badge tone="brand">{v.discountType}</Badge>,
    },
    {
      key: "value",
      header: "Value",
      render: (v) =>
        v.discountType === "PERCENT" ? `${num(v.value)}%` : idr(v.value),
    },
    {
      key: "usage",
      header: "Usage",
      render: (v) => `${v.usedCount}/${v.usageLimit}`,
    },
    {
      key: "expiry",
      header: "Expires",
      render: (v) => (
        <span className="text-ink-500">{formatDate(v.expiryDate)}</span>
      ),
    },
  ];
