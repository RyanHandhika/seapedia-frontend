...
import {
  Card, CardBody, Button, Modal, Input, Textarea, Badge, Skeleton,
  EmptyState, ConfirmModal,
} from "@/components/ui";

const EMPTY = { label: "", recipientName: "", phone: "", fullAddress: "", isDefault: false };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold text-ink-900">Addresses</h1>
              ...
              <CardBody>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-ink-500">{a.recipientName} · {a.phone}</p>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={submit} loading={createAddress.isPending}>Save address</Button>
          </>
        }
      >
        <div className="space-y-3">
          <Input label="Label" placeholder="Home, Office…" value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} />
          <Input label="Recipient name" value={form.recipientName} onChange={(e) => setForm({ ...form, recipientName: e.target.value })} />
          <Input label="Phone" placeholder="08123456789" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
          <Textarea label="Full address" value={form.fullAddress} onChange={(e) => setForm({ ...form, fullAddress: e.target.value })} />
      ...
      <ConfirmModal
        ...
        onConfirm={async () => { if (toDelete) await deleteAddress.mutateAsync(toDelete); setToDelete(null); }}
